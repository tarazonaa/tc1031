...
void eliminarPar() {
    struct nodo *indiceAct = primero;
    struct nodo *indiceAnt = NULL;
    struct nodo *temporal;
    while(indiceAct != NULL) {
        if(indiceAct->valor % 2 == 0) {
            if(indiceAnt == NULL) {
                temporal = indiceAct;
                primero = indiceAct->siguiente;
                printf("Eliminando: %d\n",temporal->valor);
                free(temporal);
                indiceAct = primero;
            } else {
                temporal = indiceAct;
                indiceAnt->siguiente = indiceAct->siguiente;
                printf("Eliminando: %d\n",temporal->valor);
                free(temporal);
                indiceAct = indiceAnt->siguiente;
            }
        } else {
            indiceAnt = indiceAct;
            indiceAct = indiceAct->siguiente;
        }
    }
}
