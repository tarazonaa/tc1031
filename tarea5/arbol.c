...
int pairNodeSum(struct nodo *inicio) {
    if (inicio == NULL) {
        return 0;
    } else {
        if (inicio->valor %2 == 0)
            return 1 + pairNodeSum(inicio->izq) + pairNodeSum(inicio->der);
        else 
            return pairNodeSum(inicio->izq) + pairNodeSum(inicio->der);
    }
}

int oddNodeSum(struct nodo *inicio) {
    if (inicio == NULL) {
        return 0;
    } else {
        if (inicio->valor %2 != 0)
